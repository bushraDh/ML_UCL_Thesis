...
class CustomDataset(Dataset):
    def __init__(self, csv_file, root_dir, transform=None, target_size=(256, 256), sequence_length=10):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.sequence_length):
            ...
            label = self.labels_df.iloc[idx + i, -2:].values.astype(float)  # Ensure label is of type float
...
def load_datasets(label_dirs, image_dirs, runs, type='CoB', transform=None, sequence_length=10):
    datasets = []
    for i in range(runs):
        ...
        if csv_path.endswith(".csv"):
            ...
            dataset = CustomDataset(csv_file=file, root_dir=image_dir, transform=transform, sequence_length=sequence_length)
...
if __name__ == "__main__":
    ...
    datasets = load_datasets(label_dirs, image_dirs, runs=509, type='CoM', transform=transform, sequence_length=sequence_length)
