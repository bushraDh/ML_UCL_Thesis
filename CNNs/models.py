...
class HeatmapRegressionModel(nn.Module):
    def __init__(self):
        super(HeatmapRegressionModel, self).__init__()
        # Define a simple CNN with a few convolutional layers
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1)
        self.conv4 = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, padding=1)  # Assuming heatmap has 1 channel
        self.relu = nn.ReLU()
...
def CNN1(input_shape=(1, 256, 256)):
